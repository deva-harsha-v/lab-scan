  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: tokens.bgPage, color: tokens.textPrimary, fontFamily: font.sans, position: 'relative', overflowX: 'hidden' }}>
        ...
        {/* Navbar */}
        <nav style={{ position:'sticky', top:0, zIndex:50, height:68, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', background:tokens.bgNav, backdropFilter:tokens.backdropBlur, borderBottom:`1px solid ${tokens.border}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <LabScanLogo size={30} gradientId="hodDashLogoGrad" />
            <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:18, color:tokens.textPrimary }}>LabScan</span>
            <span style={{ marginLeft:6, padding:'3px 10px', borderRadius:6, background:tokens.warningBg, border:`1px solid ${tokens.warningBorder}`, fontFamily:font.mono, fontSize:10, fontWeight:700, color:tokens.warning }}>HOD PORTAL</span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:24 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, background:tokens.bgSurface, padding:'6px 14px', borderRadius:20, border:`1px solid ${tokens.border}` }}>
              ...
              <span style={{ fontSize:11, fontFamily:font.mono, color:tokens.textSecondary }}>Online</span>
              <span style={{ fontSize:11, fontFamily:font.mono, color:tokens.textDimmer, marginLeft:4 }}>Sync: {systemSync}</span>
            </div>
            <ThemeToggle size="sm" />
            <button className="interactive-btn-ghost" style={{ padding:'8px 16px', background:tokens.dangerBg, border:`1px solid ${tokens.dangerBorder}`, borderRadius:8, color:tokens.danger, fontFamily:font.mono, fontSize:11, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:6 }} onClick={() => { logout(); navigate('landing'); }}>
              <LogOut size={13}/> Sign Out
            </button>
          </div>
        </nav>

        <div style={{ padding:'clamp(16px,4vw,48px) clamp(16px,4vw,48px) 80px', maxWidth:1240, margin:'0 auto', position:'relative', zIndex:1 }}>

          {/* Hero */}
          <div style={{ padding:'32px', borderRadius:'24px', background:tokens.bgCard, border:`1px solid ${tokens.border}`, marginBottom:'32px', backdropFilter:tokens.backdropBlur, display:'grid', gridTemplateColumns:'1fr auto', gap:24, alignItems:'center', boxShadow:tokens.shadow }}>
            <div>
              <div style={{ fontFamily:font.mono, fontSize:11, letterSpacing:'0.2em', color:tokens.accent, textTransform:'uppercase', marginBottom:6, fontWeight:700 }}>HOD Portal</div>
              <h1 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'32px', color:tokens.textPrimary, marginBottom:'8px' }}>Student Enrollment Center</h1>
              <p style={{ fontSize:'14px', color:tokens.textSecondary, lineHeight:'1.6' }}>Manage sections, subjects, faculty assignments, and student enrollment.</p>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10, background:tokens.bgSurface, padding:12, borderRadius:18, border:`1px solid ${tokens.border}` }}>
              {[
          ...
          {/* Tab Bar */}
          <div style={{ display:'flex', gap:8, background:tokens.bgSurfaceHover, padding:6, borderRadius:30, marginBottom:32, border:`1px solid ${tokens.border}`, overflowX:'auto' }}>
            {TABS.map(t => {
              const Icon = t.icon;
              const active = tab === t.id;
              return (
                <button key={t.id} className={`tab-btn ${active ? 'tab-btn-active' : ''}`} onClick={() => setTab(t.id)}
                  style={{ padding:'10px 22px', background:'none', border:'1px solid transparent', borderRadius:24, color:active ? tokens.accent : tokens.textSecondary, fontFamily:font.mono, fontSize:12, fontWeight:700, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:8, whiteSpace:'nowrap' }}>
          ...
          {/* Tab Content */}
          {tab === 'overview' && (
            <div style={{ display:'flex', flexDirection:'column', gap:32, animation:'fadeUp 0.4s ease forwards' }}>
