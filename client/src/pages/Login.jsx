...
export default function Login() {
  ...
  const cardStyle = {
    flex:1, minWidth:'clamp(280px, 100%, 380px)', maxWidth:380,
  ...
  return (
    <div style={{ minHeight:'100vh', background:tokens.bgPage, color:tokens.textPrimary, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 24px', fontFamily:"'DM Sans', sans-serif", position:'relative', overflow:'hidden', transition:'background 0.3s ease, color 0.3s ease' }}>
      ...
      {/* Brand header */}
      <div style={{ textAlign:'center', marginBottom:'clamp(24px, 5vw, 40px)', zIndex:1, position:'relative' }}>
        <div onClick={() => navigate('landing')} style={{ display:'inline-flex', justifyContent:'center', cursor:'pointer', marginBottom:16 }}>
          <svg width="clamp(40px, 10vw, 52px)" height="clamp(40px, 10vw, 52px)" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg"
        ...
        <h1 style={{ fontFamily:"'Syne', sans-serif", fontWeight:800, fontSize:'clamp(24px, 5vw, 36px)', letterSpacing:'-0.02em', color:tokens.textPrimary, textShadow: isDark ? `0 0 12px ${tokens.accent}40` : 'none' }}>LabScan</h1>
        <p style={{ fontFamily:"'Space Mono', monospace", fontSize:'clamp(10px, 2vw, 11px)', color:tokens.textMuted, letterSpacing:'0.08em', marginTop:8 }}>Laboratory Attendance &amp; Evaluation System</p>
      </div>

      {/* Dual panels */}
      <div style={{ display:'flex', gap:'clamp(16px, 4vw, 32px)', flexWrap:'wrap', justifyContent:'center', zIndex:1, position:'relative', width:'100%', maxWidth:800 }}>
        {/* Faculty */}
        <div style={cardStyle}
