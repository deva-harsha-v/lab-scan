...
export default function StudentDashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:tokens.bgPage, color:tokens.textPrimary, fontFamily:"'DM Sans',sans-serif", position:'relative', transition:'background 0.3s ease, color 0.3s ease' }}>
      ...
      {/* Nav */}
      <nav style={{ ...S.nav, padding:'0 clamp(12px, 3vw, 24px)', height:'clamp(56px, 12vw, 64px)', background: tokens.bgNav, borderBottom: `1px solid ${tokens.border}`, backdropFilter: tokens.backdropBlur, flexWrap: 'wrap', justifyContent: 'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'clamp(8px, 2vw, 12px)', cursor:'pointer' }} onClick={() => navigate('landing')}>
          <LabScanLogo size={28} gradientId="studentDashLogoGrad" />
          <span style={{ fontSize:'clamp(14px, 3vw, 18px)', fontWeight:800, letterSpacing:'-0.02em', color: tokens.textPrimary }}>LabScan</span>
      ...
      <div style={{ padding:'clamp(16px,4vw,40px) clamp(16px,4vw,40px) 80px', width:'100%', position:'relative', zIndex:1, animation:'fadeIn 0.5s ease forwards', display:'flex', flexDirection:'column', gap:'clamp(16px, 4vw, 28px)' }}>
        {err && <div style={S.errorBanner}>{err}</div>}

        {/* Hero */}
        <div style={{ padding:'clamp(20px, 4vw, 36px)', borderRadius:24, background: tokens.bgCard, border:`1px solid ${tokens.border}`, display:'flex', flexDirection:'column', gap:'clamp(12px, 3vw, 20px)', boxShadow: tokens.shadowCard }}>
          <div>
            <h1 style={{ fontSize:'clamp(20px, 5vw, 28px)', fontWeight:800, color:tokens.textPrimary, letterSpacing:'-0.02em', marginBottom:6 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'clamp(12px, 3vw, 20px)' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            {targetLab ? (
              <div className="std-card" style={{ ...S.card, borderRadius:20, padding:'clamp(16px, 4vw, 28px)', display:'flex', flexDirection:'column', gap:'clamp(12px, 3vw, 20px)', position:'relative', boxShadow: tokens.shadowCard }}>
